...
>
    <div class="text-center mb-10">
        ...
        <a
            ...
            @click.prevent="showInbox = true; selectedMessage = {}"
        >
            <h2
                class="inline-block text-lg border-2 p-1 rounded w-[175px] hover:border-accent-blue hover:text-white"
                :class="showInbox ? 'border-accent-blue' : ''"
            >
                I Received
            </h2>
        </a>
        <a
            class="no-underline cursor-pointer"
            @click.prevent="showInbox = false; selectedMessage = {}"
        >
            <h2
                class="inline-block text-lg border-2 p-1 rounded w-[175px] hover:border-accent-blue hover:text-white"
                :class="!showInbox ? 'border-accent-blue' : ''"
    ...
    <template x-if="showInbox">
        <template x-for="message in receivedMessages">
            <div
                :class="selectedMessage.id == message.id ? 'border-blue-500 border-2 shadow-blue-500' : 'shadow-gray-500 hover:scale-[1.005]'"
                class="mx-auto mb-3 border rounded w-full md:w-4/5 lg:w-2/3 xl:w-1/2 p-4 shadow-md transform ease-in-out"
            >
                <div
                    @click="resetMessageClickedState(message)"
                    :class="selectedMessage.id == message.id ? 'border-b border-dashed pb-4' : ''"
                    class="flex items-center cursor-pointer"
                >
                    <img
                        :src="message.sender_user.profile.avatar.customPath ? message.sender_user.profile.avatar.customPath : message.sender_user.profile.avatar.defaultPath"
                        class="w-[65px] h-[65px] rounded-full"
                    />
                    <div class="flex flex-grow pl-4">
                        <ul class="w-2/3">
                            <li
                                x-text="message.sender_user.firstname + ' ' + message.sender_user.lastname"
                            ></li>
                        ...
                        <div class="ml-auto">
                            ...
                            <svg
                                x-show="message.recipient_has_read == 0"
                                xmlns="http://www.w3.org/2000/svg"
                                class="ml-auto h-5 w-5 text-accent-blue"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                            >
                                <path
                            ...
                            <svg
                                x-show="message.recipient_has_read == 1"
                                xmlns="http://www.w3.org/2000/svg"
                ...
                <div
                    x-cloak
                    x-show="selectedMessage.id == message.id"
                    x-collapse
                    x-transition
                    x-transition:leave.delay="0"
                    class="mt-6 px-2 md:px-12 py-6 cursor-default"
                >
                    ...
                    <div class="flex flex-col">
                        <template x-if="message.replies">
                            <template
                                x-for="reply in message.replies.reply_trail"
                            >
                                <div
                                    :class="message.sender_id == reply.sender_id ? 'self-start text-left' : 'self-end text-right'"
                                    class="rounded border w-full md:w-3/4 my-2 p-2"
                                >
                                    <small
                                        class="block"
                                        x-text="reply.human_created_at"
                                    ></small>
                                    <small
                                        class="block"
                                        x-text="reply.human_created_at_time"
                                    ></small>
                                    <q
                    ...
                    <div class="flex justify-end gap-x-2 mt-4">
                        <button
                            x-show="!hasClickedReplyBtn"
                            @click="hasClickedDeleteBtn = true"
                            class="app-btn app-btn-secondary"
                            :disabled="hasClickedDeleteBtn"
                        >
                            Delete
                        </button>
                        <button
                            x-show="!hasClickedDeleteBtn"
                            @click="hasClickedReplyBtn = true"
                            class="app-btn app-btn-primary"
                            :disabled="hasClickedReplyBtn"
                        >
                            Reply
                        </button>
                    </div>
                    <div x-cloak x-show="hasClickedDeleteBtn">
                        <p>
                            Are you sure? You won't be able to recover it once
                            it's deleted.
                        </p>
                        <div class="flex gap-x-2 mt-2">
                            <button
                                class="app-btn app-btn-secondary"
                                @click="hasClickedDeleteBtn = false"
                            >
                                Return
                            </button>
                            <button
                                class="app-btn app-btn-primary"
                                @click="confirmDeletePressed"
                            >
                                Yes, I'm sure
                            </button>
                        </div>
                    ...
                    <div x-cloak x-show="hasClickedReplyBtn" class="mt-4">
                        <textarea
                            rows="5"
                            class="w-full rounded text-slate-700"
                        ...
                        <div class="flex gap-x-2 mt-2">
                            <button
                                class="app-btn app-btn-secondary"
                                @click="hasClickedReplyBtn = false"
                            >
                                Return
                            </button>
                            <button
                                class="app-btn app-btn-primary"
                                @click="confirmReplyPressed"
                            >
                                Send
                            </button>
                        </div>
    ...
    <template x-if="!showInbox">
        <template x-for="message in sentMessages">
            ...
            >
                ...
                >
                    <img
                        :src="message.recipient_user.profile.avatar.customPath ? message.recipient_user.profile.avatar.customPath : message.recipient_user.profile.avatar.defaultPath"
                        class="w-[65px] h-[65px] rounded-full"
                    />
                    <div class="flex flex-grow pl-4">
                        <ul class="w-2/3">
                            <li
                                x-text="message.recipient_user.firstname + ' ' + message.recipient_user.lastname"
                            ></li>
                            <li x-text="message.subject"></li>
                        </ul>
                        <div class="ml-auto">
                            <span
                                class="ml-auto"
                                x-text="message.human_created_at"
                            ></span>
                            <svg
                                x-show="message.sender_has_read == 0"
                                xmlns="http://www.w3.org/2000/svg"
                                class="h-5 w-5 ml-auto text-accent-blue"
                                viewBox="0 0 20 20"
                                fill="currentColor"
                            >
                                <path
                            ...
                            <svg
                                x-show="message.sender_has_read == 1"
                                xmlns="http://www.w3.org/2000/svg"
                ...
                <div
                    ...
                    x-transition
                    x-transition:leave.delay="0"
                    class="mt-6 px-12 py-6 cursor-default"
                >
                    <div class="rounded min-h-[150px]">
                        <p>
                            <q class="whitespace-pre-wrap" x-text="message.message"></q>
                    ...
                    <div class="flex justify-end gap-x-2 mt-4">
                        <button
                            x-show="!hasClickedOutboxReplyBtn"
                            @click="hasClickedOutboxDeleteBtn = true"
                            class="app-btn app-btn-secondary"
                            :disabled="hasClickedOutboxDeleteBtn"
                        >
                            Delete
                        </button>
                        <button
                            x-show="!hasClickedOutboxDeleteBtn"
                            @click="hasClickedOutboxReplyBtn = true"
                            class="app-btn app-btn-primary"
                            :disabled="hasClickedOutboxReplyBtn"
                        >
                            Reply
                        </button>
                    </div>
                    <div x-cloak x-show="hasClickedOutboxDeleteBtn">
                        <p>
                            Are you sure? This chain will be removed and you
                            won't be able to retrieve it.
                        </p>
                        <div class="flex gap-x-2 mt-2">
                            <button
                                class="app-btn app-btn-secondary"
                                @click="hasClickedOutboxDeleteBtn = false"
                            >
                                Return
                            </button>
                            <button
                                class="app-btn app-btn-primary"
                                @click="confirmOutboxDeletePressed"
                            >
                                Yes, I'm sure
                            </button>
                        </div>
                    ...
                    <div x-cloak x-show="hasClickedOutboxReplyBtn" class="mt-4">
                        ...
                        <div class="flex gap-x-2 mt-2">
                            <button
                                class="app-btn app-btn-secondary"
                                @click="hasClickedOutboxReplyBtn = false"
                            >
                                Return
                            </button>
                            <button
